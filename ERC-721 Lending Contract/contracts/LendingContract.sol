    ...
    IERC721Enumerable
{
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        uint256 _index = _tokenOwners.length() - 1;
        _tokenIDToTokenIndex[tokenId] = _index;

        emit Transfer(address(0), to, tokenId);
    }

    function getTokenIndexByID(uint256 _tokenId)
        internal
        virtual
        returns (uint256)
    {
...
contract DBNFT is ERC721 {
    ...
    function calcLoan(uint256 _tokenId, uint256 _periodInSeconds) public returns (uint256) {
        uint256 _index = getTokenIndexByID(_tokenId);
    ...
    function borrowNFT(uint _tokenId, uint _periodInSeconds) payable public {
        require(msg.sender != address(0));
        require(_exists(_tokenId),"nonexistant token");
        uint _index = getTokenIndexByID(_tokenId);
        require(getNFTInfobyID[_index].loanStatus == LoanStatus.FOR_LOAN, "NFT is not available for loan");
        uint loanAmount = calcLoan(_tokenId, _periodInSeconds);
    ...
    function buyNFT(uint _tokenId, address payable _newOwner) payable public {
        require(msg.sender != address(0));
        require(_exists(_tokenId),"nonexistant token");
        uint _index = getTokenIndexByID(_tokenId);
        DB memory _db = getNFTInfobyID[_index];
        //Check the status of NFT and transferred value
        require(_db.saleStatus == SaleStatus.FOR_SALE, "NFT is not available for sale");
        require(msg.value >= _db.value, "Please send atleast the sale amount to buy");
    ...
    function liquidateNFT(uint _tokenId, address payable _receiveAddress) public {
        require(msg.sender != address(0));
        require(_exists(_tokenId),"nonexistant token");
        uint _index = getTokenIndexByID(_tokenId);
        DB memory _db = getNFTInfobyID[_index];
        ...
        require((block.timestamp - _db.onLoanFrom)>_db.loanPeriod, "NFT still within Loan period, cannot liquidate");
    ...
    function returnNFT(uint _tokenId, address payable _receiveAddress) payable public {
        require(msg.sender != address(0));
        require(_exists(_tokenId),"nonexistant token");
        uint _index = getTokenIndexByID(_tokenId);
        DB memory _db = getNFTInfobyID[_index];
        //Loan Period is still valid - not liquidated by the owner 
        require(_db.loanStatus == LoanStatus.ON_LOAN, "Loan period voilated, so NFT is liquidated");
        require((block.timestamp - _db.onLoanFrom)<=_db.loanPeriod, "Loan period voilated, cannot return NFT");
        ...
        uint deposit = 2*getNFTInfobyID[_index].value;
